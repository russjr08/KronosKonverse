...
public class NetworkTestClass implements INetworkHandler {
    ...
    public NetworkTestClass() {
        ...
        try {
            ...
            // Start the connection to the com.kronosad.konverse.server. Actually, more like start receiving Packets from the com.kronosad.konverse.server.
            network.connect();
    ...
    public void onPacketReceived(Packet packet, String response) {
        ...
        switch (packet.getId()) {
            case 1:
                ...
                // Assigns our bot's 'user' to the one from the com.kronosad.konverse.server.
            ...
            case 2:
                ...
                // Okay, we've got the chat packet from the com.kronosad.konverse.server. Lets read the message and make a decision
                ...
                if (chatMessage.getChat().getMessage().startsWith("!")) {
                    ...
                    if (sending) {
                        ...
                        // Send the Packet to the com.kronosad.konverse.server, this could throw a network error, so we try/catch it.
                        try {
            ...
            case 4:
                // Oh no! We've been told to disconnect from the com.kronosad.konverse.server! (And we have...), lets honor it while
                ...
                System.out.println("I've been kicked from the com.kronosad.konverse.server: " + disconnect.getMessage());
                break;
            default:
                // We don't know how to handle the Packet. Do something here.
                System.out.println("Unexpected Packet received from com.kronosad.konverse.server!" + packet);
                break;
